...
# run app
java -jar target/test-java-app-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/test-java-app:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/test-java-app -n kaiser

# get external endpoint
kubectl get svc test-java-app-svc -n kaiser
```
...
```bash
kubectl port-forward svc/test-java-app-svc 8080:80 -n kaiser
...
kubectl rollout restart deploy/test-java-app -n kaiser
kubectl rollout status deploy/test-java-app -n kaiser
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/testjavaapp/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© rajesh.x2.mani@kp.org. For demo/PoV purposes.
